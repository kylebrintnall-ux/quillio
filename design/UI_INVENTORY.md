...
> **AMENDED 2026-08-28 by branch `rebrand/cleanup`.** This file records the state
> *before* that branch. Three things it describes have since changed, and the
> descriptions below are left as they were so the before/after is legible:
>
> - **§1** — `app.html` and `settings.html` no longer declare two `:root` blocks.
>   Each now has one, carrying the effective value of every token. The only token
>   the two blocks shared was `--ink`; the v8 `#1A1A2E` was winning and is what
>   the merged block holds. `--ink: #111` reached zero elements and is gone.
> - **§25 / §26** — nine colour literals fell to zero, and 56 unreachable selectors
>   (64 rule blocks, three of them `@media` wrappers holding nothing else) were
>   deleted. Two further rules were rewritten rather than deleted: `.cf-meta b`
>   and the `.meta-date, .meta-assets` pair, which was split to keep `.meta-date`.
>   `design/COLOR_MAP.md` carries the colour counts before and after.
> - **§27** — `var(--fg)` and `var(--card, #f6f6f6)` are now declared. `--fg` is
>   `var(--ink)`; `.cf-meta b` was pinned to `color: inherit` because it was the
>   one reader that resolved to something else.
>
> Two entries in **§26 were wrong** and were *not* acted on: `h1` and `h2` in
> `app.html` are element selectors, so they match `.brief-headline` and
> `.output-title` despite the class. `h2 { text-transform: uppercase }` is what
> renders every screen title in caps — deleting it would have been a visual
