...
async function generateDraft(docId, direction, clients, tenantId, targets) {
  ...
  const lookupDirection = await getAssetDirections(tenantId);
  // `targets` (optional [{assetType, fieldName}]) scopes the draft to those
  // fields; undefined → whole-doc, exactly as before.
  return getDestination().generateDraft(docId, direction, clients, voiceGuide, lookupDirection, targets);
}
